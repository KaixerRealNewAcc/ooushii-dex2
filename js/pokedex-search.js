var PokedexSearchPanel = Panels.Panel.extend({
	...
	click: function(e) {
		...
		if (e.target.tagName === 'BUTTON' && e.target.name === 'encounterlist') {
			e.preventDefault();
			e.stopPropagation();
			this.app.go('encounterlist/', this, false, $(e.target));
			return;
		}
	...
	find: function(val) {
		...
		if (!val) val = '';
		// In Encounters, exact Pokemon query becomes a Pokemon filter chip.
		if (this.search.qType === 'location' && !this.search.filters) {
			var qid = toID(val.trim());
			if (qid && BattlePokedex[qid]) {
				this.search.engine.addFilter(['pokemon', qid]);
